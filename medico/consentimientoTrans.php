<?php
	...
	if(isset($_REQUEST['enviar']))
	{
		...
		if (isset($_POST['componentesSang']))
		{
			$componentesSang=utf8_decode($_POST['componentesSang']);
			$componentesSang=addslashes($componentesSang);
		}
		if (isset($_POST['productos']))
		{
			$productos=utf8_decode($_POST['productos']);
			$productos=addslashes($productos);
		}
		...
		$queryInsConsenTransfu = "INSERT INTO consentimientotransfurg (id,numeroExpediente,folio,fechaQx,diagnostico,componentesSang,productos,
								alternativas,beneficios,nombreMedicoTratante,cedula,usr)
							VALUES (NULL,'$expediente','$folio','$fecha','$diagnostico','$componentesSang','$productos','$alternativas','$beneficios',
									'$nombreMedicoTratante','$cedula','$rol')";
		$result0 = mysqli_query($conexionMedico, $queryInsConsenTransfu);
		if(!$result0) {
			echo '!<br> ERROR al insertar Consentimiento para transfusión! <br>';
			echo 'QUERY: '.$queryInsConsenTransfu;
		} else {
			echo '<br>!!!! SE GUARDO EL CONSENTIMIENTO PARA TRANSFUSIÓN CORRECTAMENTE !!!!<br>';
		}
                ...
                <div class="row">
                    <div class="col-sm-10 col-sm-offset-1 col-lg-10 col-lg-offset-13 col-md-10 col-md-offset-1">
                    	...
                    	<form role="form" action="" method="post">
                    		...
                    		<h3>CONSENTIMIENTO INFORMADO PARA TRANSFUSIÓN</h3>
                    		<p></p>
                    		...
                    		<fieldset>
								...
								<div class="form-group">
									<label>PADECIMIENTO ACTUAL (DIAGNÓSTICO) : <span>*</span></label>
									<textarea class="form-control required" name="diagnostico" id="diagnostico" cols="10" rows="3"><?php echo $diagFin ?></textarea>
								</div>
								
								<div class="form-group">
                    			    <label>COMPONENTE(S) SANGUINEO(S) SOLICITADO(S) : </label>
                                    <textarea class="form-control" name="componentesSang" id="componentesSang" cols="10" rows="3"></textarea>
                                </div>
								<div class="form-group">
                    			    <label>TRANSFUSIÓN DE LOS PRODUCTOS: </label>
                                    <textarea class="form-control" name="productos" id="productos" cols="10" rows="3"></textarea>
                                </div>
								<h4>Información Personalizada (En base a edad, peso y antecedentes)</h4>
								<div class="form-group">
