	...
	if(isset($_REQUEST['enviar']))
	{
	    ...
	    if (empty($resultado[0])) {
		    ...
		    echo '<br/><p class="auto-style7"> <strong> Favor de seleccionar una fila:</strong>
				...
				'; 
			$vista = '../medicoHospital/solImagen.php';
		    for($i=0; $i<count($resultado); $i++) {
				for($j=0; $j<count($resultado[$i]); $j++) {
				    ...
				    echo '<form method="post" action="'.$vista.'"><tr>
							...
							<td> <input class="btn-success" type="submit" name="enviarQX" style="height: 62px; width: 100px" /></span></td>
							<input type="hidden" name="rol" value="'.$rol.'" />
							<input type="hidden" name="permisos" value="'.$permisos.'" />
							<input type="hidden" name="exp" value="'.$expediente_pac.'" />
...
<title>VISTA QUIROFANO IMG</title>
...
<body class="styleBD" onload="reportes(<?php echo $permisos ?>);" >
	...
	<?php if( $permisos == '3'){ ?>
		<p class="auto-style7" ><strong>BIENVENIDO(A) A SOLICITUDES PARA IMAGENOLOGÍA<br/></strong>
	<?php } else { ?>
			<p class="auto-style7" ><strong>BIENVENIDO(A) AL APARTADO DE TRIAGE<br/></strong>
	<?php } ?>
	
	<br>
	<span> <strong>FAVOR DE DE COLOCAR EL DATO SOLICITADO</strong> </span></p>
...
<!----------------------Primera Opcion BUSCAR POR NUM DE EXPEDIENTE---------------------------------------------------------->
	...
	<p class="auto-style5" > <input class="btn btn-danger" type="button" value="CERRAR" onClick="window.close()" style="width: 137px; height: 44px" /></p>
</body>
