            ...
            <div class="container">
                <!--div class="row"-->
                    <!--div class="col-sm-20"-->
						...
						-->
							<h1>HISTÓRICO NOTAS URGENCIAS</h1>
						...
						<table id="simple" class="display">
							...
							<tbody>
								...
								<tr id="nota">
									...
									<td><?php echo utf8_encode($row['antecedentes']) ?></td>
									<td><?php echo utf8_encode($row['interrogatorio']) ?></td>
									...
									<td style="display: none"><?php echo utf8_encode($row['resEst']) ?></td>
									<td style="display: none"><?php echo utf8_encode($row['diag']) ?></td>
									<td style="display: none"><?php echo utf8_encode($row['tratamientoFin']) ?></td>
									...
									<td style="display: none"><?php echo $row['horaFin'] ?></td>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
			  ...
			  $('#simple').on('click', '#nota', function () {
				...
				resEst = $(nTds[16]).text();
				diagn  = $(nTds[17]).text();
			 	tratam2  = $(nTds[18]).text();
				cedula  = $(nTds[19]).text();
				pronosticoVida  = $(nTds[20]).text();
				pronosticoFuncion  = $(nTds[21]).text();
				horaFin  = $(nTds[22]).text();
				glucosa  = $(nTds[23]).text();
				...
				$("#div_User").load("modificarNota.php",{idNotaUrg:idNotaUrg,hora:hora,antece:antece,resEst:resEst,inter:inter,fc:fc,fr:fr,ta:ta,temp:temp,so:so,glucosa:glucosa,habEx:habEx,cabez:cabez,torax:torax,abdom:abdom,extrm:extrm,diagn:diagn,tratam2:tratam2,cedula:cedula,pronosticoVida:pronosticoVida,pronosticoFuncion:pronosticoFuncion,horaFin:horaFin} ,function(){
