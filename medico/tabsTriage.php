<?php
	...
	$queryNotUr = 'SELECT * FROM notaurgtriage WHERE numeroExpediente='.$expediente_pac.' AND folio='.$folio_pac;
                $nota = mysqli_query($conexionMedico, $queryNotUr) or die (mysqli_error($conexion));
                $rowN = mysqli_fetch_array($nota);
	$desc=null;
	if($rowN!= NULL){
    	$desc = "<span style='color:red'>ESTE PACIENTE CON EL NÚMERO DE FOLIO SELECCIONADO, YA POSEE DATOS DE TRIAGE</span>";
	}

	$diagTriage=NULL;
	$antecOld=NULL;

	//Query para ver el diagnostico
	$queryAntec = "SELECT *
				  FROM notaUrg
				  WHERE numeroExpediente='$expediente' AND folio='$folio_pac' AND estatus='1'
				  ORDER BY fecha DESC
				  LIMIT 1";

	$antec = mysqli_query($conexionMedico, $queryAntec) or die (mysqli_error($conexionMedico));
	while($rowA = mysqli_fetch_array($antec)){
		$diagTriageOld = utf8_encode($rowA['diag']);
		$diagTriage = addslashes ($diagTriageOld);
	}
	//No tiene diagnostico en consulta, verificamos si tiene en urgencias
	if($diagTriage == NULL || $diagTriage == ''){
		//Query para ver el diagnostico
		$queryAntec = "SELECT *
					  FROM notaUrgchoque
					  WHERE numeroExpediente='$expediente' AND folio='$folio_pac' AND estatus='1'
	...
	<body class="bg-blue">
		<div>
			<h1 style="color: aliceblue">				
				<img alt="logoHD" height="100" width="108" src="../img/logoNew2.jpg" /> TRIAGE </h1>
		</div>
		<div class="body">
			<!-- tabs -->
			<div class="sky-tabs sky-tabs-pos-top-left sky-tabs-anim-flip sky-tabs-response-to-icons">
				...
				<label for="sky-tab2"><span><span><i class="fa fa-exclamation-triangle"></i>DATOS DE TRIAGE</span></span></label>
				...
				<ul>
					<li class="sky-tab-content-1">
						<div class="typography">
							...
							<p>
								...
								<br/>
								<strong> DIAGNÓSTICO(S):&nbsp;&nbsp;</strong><?php echo $diagTriage ?> &nbsp;&nbsp;&nbsp;&nbsp;
							</p>
							<p>
					...
					<li class="sky-tab-content-2">
						<div class="typography">
							<h1>DATOS DE TRIAGE</h1>
							<h2><?php echo $desc ?></h2>
							<p>
								<input type="button" value="AGREGAR DATOS" class="btn btn-primary" name="lvc" style="height: 50px; width: 300px" onClick="window.open('notaUrgenciasTriage.php?rol=<?php echo $rol ?>&exp=<?php echo $expediente_pac ?>&folio=<?php echo $folio_pac ?>', '_blank').focus()" />
							</p>
							<p>
								<input type="button" value="CONSULTAR HISTÓRICO" class="btn btn-info" name="lvc" style="height: 50px; width: 300px"
								onClick="window.open('consultaNotaUrgT.php?rol=<?php echo $rol ?>&exp=<?php echo $expediente_pac ?>&folio=<?php echo $folio_pac ?>', '_blank').focus()"/>
							</p>
						</div>
					</li>
