...
if($nombre == 'ReporteTriage'){
	...
	$excel= "REPORTE DE TRIAGE   ".utf8_encode(strftime('%A, %d de %B de %Y'))."\n";
	...
	$result1 = mysqli_query($conexionMedico, $queryExcelFT) or die (mysqli_error($conexionMedico));
	
	#Forma POO instanciamos y mandamos llamar un objeto de la instancia
	$usuario1 = new FuncionesDB();
	
	$excel.="\nConsecutivo\tExpediente\tFolio\tNombre Paciente\tFecha\tHora\tTurno\tAcude\tMotivo\tTA\tFC\tFR\tTemperatura\tSO\tPeso\tTalla\tColor\tRealizo\n";
	$c = 1;
	$d = 0;
	while($row = mysqli_fetch_array($result1)){
		$nombre_pac = NULL;
		#La funcion retorna un arreglo lo mandamos a una variable
		$resultado[] = $usuario1->consultaBasicos($row['numeroExpediente'],$row['folio']);
		#El arreglo esta vacio
		if (!empty($resultado[0])) {
			$nombre_pac = utf8_decode($resultado[$d][0]['NOMBRE']);
		}
		$turno='';
		if($row['turno']=='M'){
			$turno='MATUTINO';
		...
		$excel.= $c++."\t".$row["numeroExpediente"]."\t".$row["folio"]."\t".utf8_encode($nombre_pac)."\t".$row["fecha"]."\t".$row["hora"]."\t".$turno."\t".utf8_encode($row["acude"])."\t".utf8_encode($row["motivo"])."\t".$row["ta"]."\t".$row["fc"]."\t".$row["fr"]."\t".$row["temp"]."\t".$row["so"]."\t".$row["peso"]."\t".$row["talla"]."\t".$row["color"]."\t".utf8_encode($row["realizo"])."\n";
		$d++;
	}
}
...
if($nombre == 'RevisionConsumiblesaHigieneManos'){
	...
	while($row = mysqli_fetch_array($result1)){
		$fila1[] = $row;
		if($c == 1){
			$excel.="\nConsecutivo\tVerificador\tFecha Verificación\tLocalización del Dispositivo\tHabitación\tOtra Ubicación\tTurno\n";
		}
...
if($nombre == 'AccesosVascPerInst'){
	...
	while($row = mysqli_fetch_array($result1)){
		...
		if($c == 1){
			...
			$excel.="\nConsecutivo\tVerificador\tFecha Instalación\tUBICACIÓN\tHabitación\tOtra Ubicación\tTurno\tProfesional Instaló\tNombre Instaló\n";
		}
...
if($nombre == 'AccesosVascPerManto'){
	...
	while($row = mysqli_fetch_array($result1)){
		...
		if($c == 1){
			...
			$excel.="\nConsecutivo\tVerificador\tFecha Instalación\n";
		}		
...
if($nombre == 'VigilanciaYcapacitacion'){
	...
	while($row = mysqli_fetch_array($result1)){
		if($c == 1){
			$excel.="\nConsecutivo\tNúmero Expediente\tFolio\tFecha Visita\tHora Visita\tMeta Internacional\tVigilancia de RPBI\tInmunocomprometido\tComorbilidad\tAislamiento\tPadecimiento Actual\tnotas\n";
		}
...
if($nombre == 'MedicionesTemperatura'){
	...
	while($row = mysqli_fetch_array($result1)){
		if($c == 1){
			$excel.="\nConsecutivo\tNúmero Expediente\tFolio\tFecha Medición\tHora Medición\tTemperatura\tVerificador\tObservaciones\n";
		}
...
if($nombre == 'OtrasInfecciones'){
	...
	while($row = mysqli_fetch_array($result1)){
		if($c == 1){
			$excel.="\nConsecutivo\tNúmero Expediente\tFolio\tFecha Inicio\tFecha Observacion\tFecha Termino\tVerificador\tObservaciones\n";
		}
		$observaciones= preg_replace("/[\n|\r|\n\r]/i"," ",$row["observaciones"]);
...
if($nombre == 'AccesosVascCentrInst'){
	...
	while($row = mysqli_fetch_array($result1)){
		...
		if($c == 1){
			$excel.= "EXPEDIENTE: ".$row["numeroExpediente"]."\tSEXO: ".$sexo."\n";
			$excel.="\nConsecutivo\tFECHA DE INSTALACIÓN\tFECHA DE RETIRO\n";
		}
