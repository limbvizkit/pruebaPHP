<?php
	...
	if(!empty($idResguardo)){
		$queryUpdEvAdv = "UPDATE resguardos SET area='$area',entrega='$entrego',recibe='$recibe',cargo='$cargo',observaciones='$observaciones'
							WHERE idResguardo='$idResguardo'";
		$result0 = mysqli_query($conexion, $queryUpdEvAdv) or die (mysqli_error($conexion));
		
		if(!empty($idMaterial)){
			$queryUpdEvAdv = "UPDATE materiaresguardo SET cantidad='$cantidad',unidad='$unidad',tipoActivo='$tipoActivo',descActivo='$descActivo',
