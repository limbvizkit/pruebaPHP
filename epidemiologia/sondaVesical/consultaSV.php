...
<html lang="es">
    <head>
        ...
        <title>CONSULTA SONDA VESICAL</title>
        <!-- Google Font -->
							...
							<tbody>
								<?php
									...
									while($row = mysqli_fetch_array($docs)){
										$fecha = strtotime($row['fechaProcedimiento']);
								...
								<tr id="vig">
									...
									<td id="fechaVis" style="display: none"><?php echo $row['fechaProcedimiento'] ?></td>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
			  ...
			  $('#simple').on('click', '#vig', function () {
				...
				var nTds = $('td', this);
				idSV = $(nTds[1]).text();
				fechaProcedimiento = $(nTds[7]).text();
				...
				$("#div_User").load("modificarSV.php",{idSV:idSV,fechaProcedimiento:fechaProcedimiento,fechaFin:fechaFin,fecha1:fecha1,fecha2:fecha2,verificador:verificador,nombreInstalo:nombreInstalo,observaciones:observaciones} ,function(){
