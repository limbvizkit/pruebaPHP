...
/********************************************************Reporte de Medicamentos con sus Dosis***************************************************/
if($nombre == 'MedicamentosDosis'){
	...
	$excel= "REPORTE DE ANTIBIÓTICOS Y DOSIS   ".utf8_encode(strftime('%A, %d de %B de %Y'))."\n";
	...
	$excel.="Diagnostico\tConcomitantes\tDep De Creatinina\tMedicamentosCasa\tCultivo\tTipoMedicamento\tFecha De Inicio\tNombre De Medicamento\tSAL\tDosis\tVía De Admon\t";
	...
	$queryExcelFT = "SELECT p.numeroExpediente,nombre,edad,sexo,fechaNacimiento,alergias,peso,talla,habitacion,
						...
						WHERE p.fechaIngreso >= '$fecha1' AND p.fechaIngreso <= '$fecha2' AND t.tipoMedicamento=1
						ORDER BY p.numeroExpediente, m.nombreMedicamento";
	...
	while($row = mysqli_fetch_array($result1)){
		...
		$excel.= utf8_encode($row[13])."\t".utf8_encode($row[14])."\t".utf8_encode($row[15])."\t".utf8_encode($row[48])."\t".utf8_encode($row[49])."\t".utf8_encode($row[16])."\t".utf8_encode($row[18])."\t";
