            ...
            <div class="container">
                <div class="row">
                    <div class="col-sm-12">
						...
						-->
							<h1>ACCESOS VASCULARES CENTRALES </h1>
							...
							<tbody>
								<?php
									...
									while($row = mysqli_fetch_array($docs)){
										$fecha = strtotime($row['fechaInst']);
										$fechaFin = date('d/m/Y',$fecha);
										$fecha1 = strtotime($row['fechaRetiro']);
								...
								<tr id="vig">
									...
									<td id="metaVis"><?php echo utf8_encode($row['nInstala']) ?></td>
									<td id="vigRPBIVig"><?php echo utf8_encode($row['complicacion']) ?></td>
									<td id="inmunoCompVig"><?php echo utf8_encode($row['observaciones']) ?></td>
									<td id="fechaVis" style="display: none"><?php echo $row['fechaInst'] ?></td>
									<td id="fechaRet" style="display: none"><?php echo  $row['fechaRetiro'] ?></td>
									<?php } ?>
								</tr>
							</tbody>
						</table>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
			  ...
			  $('#simple').on('click', '#vig', function () {
				...
				var nTds = $('td', this);
				idAVC = $(nTds[1]).text();
				fechaInst = $(nTds[7]).text();
				fechaRetiro  = $(nTds[8]).text();
				fecha1  = $(nTds[2]).text();
				fecha2  = $(nTds[3]).text();
				nInstala  = $(nTds[4]).text();
				complicacion  = $(nTds[5]).text();
				...
				$("#div_User").load("modificarAVC.php",{idAVC:idAVC,fechaInst:fechaInst,fechaRetiro:fechaRetiro,fecha1:fecha1,fecha2:fecha2,nInstala:nInstala,complicacion:complicacion,observaciones:observaciones} ,function(){
