	...
	<script type="text/javascript">
		...
		function modifRes(idMat, idRes, roles){
			...
			$("#div_User").fadeIn();
				...
				$("#div_User").load("modificarResguardorh.php",{idMaterial:idMaterial,idResguardo:idResguardo,rol:rol} ,function(){
	...
	if(isset($_REQUEST['buscar']) || isset($_GET['idMaterial'])) {
		...
		while ($row0 = mysqli_fetch_array($result0)){
			...
			echo " <tr class='alt'> 
					...
					<td></td>
					<td>
					  <a class='btn btn-primary' target='_blank' name='generapdf' style='height: 50px; width: 140px' href='pdf/creaPDF1rh.php?idResguardo=$row0[0]' />GENERAR PDF </a>
					</td>
					<td> 
					<a class='btn btn-success' target='_blank' name='addMaterial' style='height: 50px; width: 140px' href='pdf/creaPDF1rh.php?idResguar=$row0[0]' />Agregar Material </a>
					</td>
					</tr>
				";
			...
			echo "<tr>
					...
					<td>
					 	<a class='btn btn-danger' onclick='return confirmSubmit()' href='buscarResguardorh.php?idMaterial=$row[1]&idResguardo=$idResguardo&condiciones=$cond64&rol=$rol' />ELIMINAR </a>
					 </td>
				</tr>";
			}
		}
...
?>
	...
	<div>
		...
		<br/>
		<form method="post" action="buscarResguardorh.php" target="_blank">
