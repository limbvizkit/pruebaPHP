...
if($nombre == 'PerfilFarmacoTerapeutico'){
	...
	$excel.="Consecutivo\tExpediente\tNombre Paciente\tEdad\tSexo\tFecha De Nacimiento\tAlergias\tPeso\tTalla\tHabitacion\tFecha De Ingreso\tFecha De Egreso\tConciliacionIngreso\tConciliacionEgreso\tConciliacionCambioServ\tConciliacionCambioMed\tMedico\tEspecialidad\t";
	...
	$excel.="Recomendación\tObservaciones\tFechaDeConciliaciónEgreso\tObservacionesEgreso\tFarmaco Interacción\tFarmaco Interacción2\tAlimento Interacción\tDescripción Del Efecto\tSeveridad\t";
	...
	$queryExcelFT = "SELECT p.numeroExpediente,nombre,edad,sexo,fechaNacimiento,alergias,peso,talla,habitacion,
						...
						m.sal, p.medicamentoCasa, p.cultivo, p.medEgre, conciliacionCambServ, conciliacionCambMed
						FROM paciente AS p
	...
	while($row = mysqli_fetch_array($result1)){
		...
		$excel.= utf8_encode($row[5])."\t".$row[6]."\t".$row[7]."\t".$row[8]."\t".$row[9]."\t".$row[10]."\t".utf8_encode($row['medicamentoCasa'])."\t".utf8_encode($row['medEgre'])."\t".utf8_encode($row['conciliacionCambServ'])."\t".utf8_encode($row['conciliacionCambMed'])."\t".utf8_encode($row[11])."\t".utf8_encode($row[12])."\t";
		...
		$excel.= utf8_encode($row[31])."\t".utf8_encode($row[32])."\t".utf8_decode($row[34])."\t".utf8_encode($row[35])."\t".utf8_encode($row[36])."\t";
