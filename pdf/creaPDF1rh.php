<?php
	...
	if(isset($_REQUEST['addMaterial']))
	{
		...
		$queryAddResguardo = "INSERT INTO resguardosrh (idResguardo, fechaResguardo, fechaTextual, area, entrega, recibe, cargo, observaciones) 
	...
	if(isset($_REQUEST['guardar']))
	{
		...
		if(isset ($_POST['idResguardo']) && !empty($_POST["idResguardo"])){
			...
			$queryAddMaterial = "INSERT INTO materiaresguardorh (idMaterial,idResguardo,cantidad,unidad,tipoActivo,descActivo,numSerie,marca,modelo,
	...
	if(isset($_GET['idMaterial']) && !empty($_GET["idMaterial"])){
		...
		$queryDelMaterial = "UPDATE materiaresguardorh SET estatus='0' WHERE idMaterial = $idMaterial";
	...
	<body  background="../img/logoNew2Agua.jpg">
			...
			while($row = mysqli_fetch_array($idMaterial)){
				echo '<tr>
						...
						if($row['idImagen'] != NULL && $row['idImagen'] != ''){
							...
							echo'	<form enctype="multipart/form-data" action="uploaderrh.php" method="post" target="_blank">
					...
					echo '</td>
					<td class="auto-style4">
					 &nbsp;&nbsp;&nbsp;&nbsp;<a class="btn btn-danger" onclick="return confirmSubmit()" href="creaPDF1rh.php?idMaterial='.$row[0].'&idResguardo='.$idResguardo.'" />ELIMINAR </a></td>
						</tr>';
			}
		echo '</table>
			<br />
			<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <br />
			<form method="post" action="creaPDF1rh.php">
		...
		<br/>
		<form method="post" action="creaPDF1rh.php" target="_blank">
			<input name="fecha" type="hidden" value="'.$fecha.'" />
			<input name="fecha1 " type="hidden" value="'.$fecha1.'" />
			<input name="area" type="hidden" value="'.$area.'" />
			<input name="entrega" type="hidden" value="'.$entrega.'" />
...
if(isset($_REQUEST['generapdf']) || (isset ($_GET['idResguardo']) && !empty($_GET["idResguardo"]))) {
	...
	$queryCuadro1 = "SELECT * FROM resguardosrh WHERE idResguardo = $idResguardo";
	...
	while($rowM = mysqli_fetch_array($idMaterial)){
		...
		##Observaciones de la BD
	 	...
	 	$pdf->Cell(0,7,utf8_decode('Lic. Erika Hernández Piedra                         Quien Recibe Nombre y Firma'),0,1,'C');
	 	$pdf->Cell(0,7,utf8_decode('SubDirectora de Recursos Humanos                                                                           '),0,1,'C');
