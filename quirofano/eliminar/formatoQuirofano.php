...
<?php 
	...
	/*$queryInciDia0 = "SELECT fechaAlta, a1.nombreArea, a2.nombreArea, reporte, solucion, resuelto, fechaSolucion, h.numeroHabitacion, h.tipoHabitacion, i.idIncidencia
		...
		while($row = mysqli_fetch_array($resultado)){
			...
			// write text
			$sect->writeText('                      HOJA DE REGISTRO DE CIRUGÍA', $font);
			$sect->writeText('<br /><br />');
			$sect->writeText('FECHA DE CIRUGÍA: '.$date1.'                       HORA DE CIRUGÍA: '.$date2.' hrs.', $fontA);
			$sect->writeText('<br /><br />');
...
Trabajo reportado                  Enterado para realizar trabajo           Enterado para realizar trabajo',
 $font, new PHPRtfLite_ParFormat());*/
	...
	$image_11 = $cell_11->addImage('../../img/logoNew2.jpg');
	// image width 2cm and height 2cm
	$image_11->setWidth(2);
	$image_11->setHeight(2);	
		
	$query1="SELECT * FROM datosnuevos WHERE id='$id'";
	...
	while($row1 = mysqli_fetch_array($resultado1)){
		...
		// write text
		$sect1->writeText('                      HOJA DE REGISTRO DE CIRUGÍA', $font1);
		$sect1->writeText('<br /><br />');
		$sect1->writeText('FECHA DE CIRUGÍA: '.$date11.'                       HORA DE CIRUGÍA: '.$date21.' hrs.', $fontA1);
		$sect1->writeText('<br /><br />');
	...
	$api1->convert([
		...
		"outputformat" => "pdf",
		"input" => "upload",
		"file" => fopen('C:\xampp\htdocs\conectaSQLSRV\quirofano\eliminar\quirofanoMini.rtf', 'r'),
	])
	->wait()
	->download('quirofanoMini.pdf');
