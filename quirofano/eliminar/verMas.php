...
<?php 
	...
	if(isset($_REQUEST['enviarQx']))
	{
		...
		$sqlValida = "SELECT * FROM programaCirugia WHERE idSala='$salaQx' AND '$fechaFinal'=fecha AND '$hora' BETWEEN hrInicio AND hrFin AND idCirugia != '$id'";
		...
		if($resultado->num_rows > 0){
			...
			$queryDts="SELECT * FROM datosnuevos WHERE id='$id'";
			...
			while($row = mysqli_fetch_array($resultado1)){
				...
				$sqlDN = "INSERT INTO log (id, idCirugia, fecha, hora, enfermera, nombrePaciente, edad, diagnostico, cirugia, equipo, descorche, imagen,
			...
			$sql = "UPDATE datosnuevos SET fecha = '$fechaFinal', hora = '$hora', enfermera= '$enfermeraQx', nombrePaciente = '$nombre', edad = '$edad',
					diagnostico = '$diagnostico', cirugia= '$cirugia',equipo = '$equipo', descorche = '$descorche', imagen = '$imagen', 
			...
			if ($conn->query($sql)==true) {
				#sacamos los datos para la suma de horas y minutos
					...
					$sqlHrFin= "SELECT ADDTIME(hora, '$hrFin') AS horaFin FROM datosnuevos WHERE id='$id' LIMIT 1";
					...
					$sqlPC = "UPDATE programaCirugia SET idSala = '$salaQx', fecha = '$fechaFinal', hrInicio='$hora', hrFin='$hrFinal' WHERE idCirugia='$id'";
	...
	$query="SELECT * FROM datosnuevos WHERE id='$id'";
...
<script type="text/javascript">
	...
	<div id="cuadro">
		...
		<div id="titulo">
		<center><h1>Modificar Registro de Quirófano</h1></center>
		</div>
						...
						<tr>
							...
							<td>
								<select id="tipoCirgQx" name="tipoCirgQx" style="width:140px; height:40px" disabled>
							        <option value="<?php echo $row['tipoDeCirugia'];?>"><?php echo $row['tipoDeCirugia'];?></option>
							...
							<td>
								<select id="costeadorQx" name="costeadorQx" style="width:140px; height:40px" disabled>
							        <option value="<?php echo $row['costeador'];?>"><?php echo $row['costeador'];?></option>
							...
							<td>
								<select id="salaQx" name="salaQx" style="width:200px; height:40px" required disabled>
							        <option value="<?php echo $row['sala'];?>">
							        				<?php if($row['sala'] == 1){
							         						echo "SALA 1";
							         						} else 
							         						if($row['sala'] == 2){
