...
<?php 
	...
	if(isset($_REQUEST['cancelar']))
	{
		...
		$query2="DELETE FROM programacirugia WHERE idCirugia='$id'";
		...
		if($resultado2 > 0){
			...
			$query="UPDATE datosnuevos SET estatusCirugia='4' WHERE id='$id'";	
			$resultado=$mysqli->query($query);
			if($resultado > 0){
				echo "<h1> Se Cancelo la Cirugía </h1>";
				//Nombre de usr que cancela la cirugia
				$queryExtr="UPDATE extras SET usrCancela='$rol', usrAutoriza='$usrAutoriza', observaciones='$observaciones' WHERE idCirugia='$id'";	
				...
				printf("Error: %s\n", $mysqli->error);
				echo "<h3> !Error! al Cancelar la Cirugía </h3>";
			}
			echo "<h1> Se Elimino Registro de Programa Cirugía </h1>";
		}else{ 
			printf("!Error!: %s\n", $mysqli->error);
			echo "<h1> !Error! al Eliminar Registro de Programa Cirugía </h1>";
		}
...
?>
		...
		<div align="center"><img alt="logoHD" src="../../img/logoNewPeque.jpg"></div>
		<center>
			<form method="post">
				<h1>Cancelar Cirugía</h1>
				...
				<select class="form-control" id="usrAutoriza" name="usrAutoriza" style="width:430px; height:40px" required >
					...
					$queryUsrAut = "SELECT u.nombreUsr, du.nombre
								...
								WHERE u.rolUsr=8 AND NOT ISNULL(du.nombre)";
