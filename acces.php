...
<meta charset="utf-8" name="viewport" content="width=device-width, minimum-scale=1.0, maximum-scale=1.0" >
  <title>Acceso Formato Farmacia</title>
	...
	if($login1 != NULL){
		...
		switch ($rol){
			...
			case '7':
				...
				header('Location: resguardosrh.php?rol='.$usuario.'&permisos='.$permisos);
				exit();
				break;
			...
			case '13':
				...
				exit();
				break;
				//usuario de resultados de analisis de laboratorioa (Clientes solo consulta)
			case '14':
				session_name($usuario);
				session_start();
				$idSesion = session_id();
				...
				$_SESSION[$usuario]="pacientesLaboratorio";
				header('Location: visorEstudios.php?rol='.$usuario.'&permisos');
				exit();
				break;
				//usuario de Laboratorio para alta de analisis
			case '15':
				session_name($usuario);
				session_start();
				$idSesion = session_id();
				$queryAudLog = "INSERT INTO auditorialogin (idLogin, idUsuario, fechaHoraIngreso, sesionId, idSistema)
						VALUES (NULL, '$idUsr', CURRENT_TIMESTAMP, '$idSesion', '14')";
				$queryAL= mysqli_query($conexion, $queryAudLog);
				if(!$queryAL){
					echo "<br/><span style='color:red'> !!!NO SE GUARDO REGISTRO PARA AUDITORIA LOGIN!!! <br/>";
					#echo $sql;
				...
				header('Location: laboratorio/adminDoc.php?rol='.$usuario.'&permisos');
				#header('Location: https://www.henridunant.com/web/laboratorio/adminDoc.php?rol='.$usuario.'&permisos');
				exit();
				break;
				//Solo consulta de registros de quirofano
			case '16':
				session_name($usuario);
				session_start();
				$idSesion = session_id();
				...
				$_SESSION[$usuario]="consultaQuirofano";
				header('Location: quirofano/eliminar/index.php?rol='.$usuario.'&permisos');
				#header('Location: https://www.henridunant.com/web/laboratorio/adminDoc.php?rol='.$usuario.'&permisos');
				exit();
				break;
