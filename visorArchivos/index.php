	...
	if(isset ($_GET['tipo'])) {
		...
		if($_GET['tipo'] == 2){
			...
			if($archivo != NULL && $archivo != '' && $idArchivo!= NULL && $idArchivo != ''){		
				...
				$queryUpdDoc = "UPDATE datosdocumentos SET estatus = '3' WHERE idDocumento = $idArchivo";
		...
		if($_GET['tipo'] == 3){
			...
			if($archivo != NULL && $archivo != '' && $idArchivo!= NULL && $idArchivo != ''){
				...
				#Cambiamos el estatus en la BD para el archivo
				$queryUpdDoc = "UPDATE datosdocumentos SET estatus = '0' WHERE idDocumento = $idArchivo";
				$result0 = mysqli_query($conexion, $queryUpdDoc);
		...
		if($_GET['tipo'] == 1){
			...
			if($archivo != NULL && $archivo != '' && $idArchivo!= NULL && $idArchivo != ''){	
				#Cambiamos el estatus en la BD para el archivo
				$queryUpdDoc = "UPDATE datosdocumentos SET estatus = '1' WHERE idDocumento = $idArchivo";
				$result0 = mysqli_query($conexion, $queryUpdDoc);
...
<div class="head">
  ...
  <div class="h">
	...
	<table id="simple">
      ...
      <tbody>
		<?php
			...
			$queryDocs = "SELECT idDocumento,areaDepto,a.nombreArea,nombreDocumento,rutaArchivo,estatus,fechaAlta
		    ...
		    if($row['estatus'] == 3){
		        ...
		        $bttAltBj='&nbsp;&nbsp;<a class="btn btn-info" onclick="return confirmSubmitAlta()" href="index.php?rol='.$rol.'&archivo='.$archiv.'&idArchivo='.$row[0].'&tipo=1" style="width: 70px; height: 30px"/> ALTA </a>';
		    } else {
		        $colorFila='style="color:black"';
		        $bttAltBj='&nbsp;&nbsp;<a class="btn btn-primary" onclick="return confirmSubmit()" href="index.php?rol='.$rol.'&archivo='.$archiv.'&idArchivo='.$row[0].'&tipo=2" style="width: 70px; height: 30px"/> BAJA </a>';
		    }
		...
		<tr <?php echo $colorFila ?> >
			...
			<td class="auto-style7"><a href="detalleDoc.php?id=<?php echo $row[0] ?>" target="_blank" > <?php echo utf8_encode($row['nombreDocumento']) ?> </a></td>
	...
	?>
	<span class="auto-style3">*Los botones en color amarillo abren el archivo de manera protegida, los verdes permiten descargar<br></span>
	</div>
	<div class="head">
		<form action="guardarArchivo.php" method="post" target="_blank">
			<h1> CARGAR ARCHIVO PARA DESCARGA/VISUALIZACIÓN</h1>
			(PDF, Word, Excel)
