<?php
	...
	if(isset($_REQUEST['entreFechas']))
	{
		...
		$query="SELECT * FROM formvacaciones WHERE fecha BETWEEN '$fechaIni' AND '$fechaFin' ";
	
		$res = mysqli_query($conexion, $query);
		//$resultado = mysqli_fetch_array($res);
	} else {
		$query="SELECT * FROM formvacaciones GROUP BY idSolicitud Order by fecha DESC";
		$res = mysqli_query($conexion, $query);
		//$resultado = mysqli_fetch_array($res);
	}
?>
...
<script type="text/javascript">	
	...
	{
		var agree=confirm("¿Está seguro de cancelar esta registro? !!!EL PROCESO ES IRREVERSIBLE!!!");
		if (agree){
		 	return true;
		}else{
		 	return false;
	...
	<div id="cuadro">
		...
		<div id="titulo">
		<center><h1>Solicitudes de Vacaciones</h1></center>
		</div>
		<div class="datagrid">
		<table class="table table-bordered" id="simple">
				...
				<tbody>
					...
					?>
						<tr <?php echo $color ?> >
							...
							<td>
								<?php $date1 = date_create_from_format('Y-m-d 00:00:00',$row['fecha'])->format('d-m-Y'); ?>
								<?php echo $date1; ?>
							</td>
							<td>
								<?php echo utf8_encode($row['nombreEmpleado']); ?>
							</td>
							<td>
								<?php echo $row['diasCorresponden']; ?>
							</td>
							<td>
								<?php echo $row['diasDisfrutar']; ?>
							</td>
							<td>
								<?php echo $row['diasPendientes']; ?>
							</td>
							<td>
								<a class="btn btn-primary" href="verMas.php?id=<?php echo $row['idSolicitud'];?>&rol=<?php echo $rol ?>">Consultar</a>
								<br />
								<?php //if($cancela) { ?>
			...
			<form method="post">
				<p><strong>Consultar Solicitudes por Rango de Fechas</strong></p>
