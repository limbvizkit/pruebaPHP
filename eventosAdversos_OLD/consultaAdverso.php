            ...
            <div class="container">
                <!--div class="row"-->
                    <!--div class="col-sm-20"-->
						...
						-->
							<h1>HISTÓRICO DE EVENTOS ADVERSOS</h1>
						...
						<table id="simple" class="display">
							...
							<tbody>
								<?php
									$queryDocs = "SELECT *
												  FROM eventoAdverso
												  WHERE estatus='1' AND numeroExpediente=''
												  ORDER BY id DESC";
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
			  ...
			  $('#simple').on('click', '#nota', function () {
				...
				fecha  = $(nTds[2]).text();
				reporta = $(nTds[3]).text();
				servicio  = $(nTds[4]).text();
				evento  = $(nTds[5]).text();				
				fechaB  = $(nTds[6]).text();
				turno  = $(nTds[7]).text();
				diag  = $(nTds[8]).text();
				tipoEvento  = $(nTds[9]).text();
				relacionado  = $(nTds[10]).text();
				alcanzoPac  = $(nTds[11]).text();
				danioPac = $(nTds[12]).text();
				como  = $(nTds[13]).text();
				porQue  = $(nTds[14]).text();	
				medicamento  = $(nTds[15]).text();
				generico  = $(nTds[16]).text();
				presentacion  = $(nTds[17]).text();
				dosis  = $(nTds[18]).text();
				viaAdmin  = $(nTds[19]).text();
				intervalo = $(nTds[20]).text();
				aim = $(nTds[21]).text();
				cidt = $(nTds[22]).text();
				ciam = $(nTds[23]).text();
				dim = $(nTds[24]).text();
				...
				ampi = $(nTds[34]).text();
				amnp = $(nTds[35]).text();
				omisionMed = $(nTds[36]).text();
				ami = $(nTds[37]).text();
				presInc = $(nTds[38]).text();
				transInc = $(nTds[39]).text();
				prepInc = $(nTds[40]).text();
				dispoInc = $(nTds[41]).text();
				tai = $(nTds[42]).text();
				vai = $(nTds[43]).text();
				adpi = $(nTds[44]).text();
				dti = $(nTds[45]).text();
